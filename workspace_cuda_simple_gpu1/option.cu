...
Option::Option( const dtype &in_S0,
        const dtype &in_K,
        const dtype &in_T,
        const dtype in_R,
...
Option::Option(Option &in_option)
{
    S0=in_option.S0; //开盘价
    K=in_option.K; //敲定价
    T=in_option.T; //成熟期
...
void Option::set_S0(dtype in_S0)
{
    S0=in_S0;
    return;
}
...
// 该函数用于切换资产价格生成方法
void Option::change_asset_func(dtype (Option::*in_func)(const dtype&))
{
    asset_func=in_func;
    return;
}
...
//1.看涨期权
dtype Option::payoff_call(const dtype &St)
{
    return max(St-K,0.0);
}

//2.看跌期权
dtype Option::payoff_put(const dtype &St)
{
    return max(K-St,0.0);
}

//Black-Scholes解析解(call/put)
dtype Option::BS_return(const bool &is_put)
{
...
//保存MC路径
void Option::save_stock_prices(dtype *stock_price, int num_save, int time_step)
{
...
//从csv文件读取MC路径
void Option::load_stock_prices(dtype *stock_price,int &sim_num,int &time_step)
{
...
//MC误差计算
void Option::compute_error(const dtype *payoff_array,int sim_num)
{
    dtype variance=0.0;
...
void Option::show_grid(dtype *grid,int sim_num,int time_step)
{
...
void Option::show_fdm_all(dtype *grid,int sim_num,int time_step)
{
    for(int i=0;i<sim_num;i++)
    {
        cout<<Smin+i*dS<<": "<<endl;
...
__constant__ dtype drift;
__constant__ dtype diffusion;
__constant__ dtype d_S0;
__constant__ dtype d_K;
...
__global__ void MC_call(curandState *states,dtype *const array)
{
    ...
    dtype asset_price=d_S0;
    //cout<<S0<<endl;
    //printf("S0: %f\n",d_S0);
    for(int i=0;i<time_step;i++)
    {
        //asset_price=asset_price*exp((R-0.5*V*V)*dt+V*sqrt(dt)*curand_normal(&local_state));
        asset_price=asset_price*exp(drift+diffusion*curand_normal(&local_state));
    }
    //dtype payoff=(asset_price-K)>0.0?(asset_price-K):0.0;
    dtype payoff=(d_K-asset_price)>-9.0?(d_K-asset_price):0.0;
    array[tid]=payoff;

    return ;
}

void Option::MC_pricing_gpu(const int &n,const int &m)
{
    //display();
    dt=T/m;
    dtype h_drift=(R-0.5*V*V)*dt;
    dtype h_diffusion=sqrt(dt)*V;
    //printf("%f , %f \n",h_drift,h_diffusion);

    cudaMemcpyToSymbol( d_S0 ,&S0 ,1*sizeof(dtype),0,cudaMemcpyHostToDevice )  ;
    cudaMemcpyToSymbol( d_K ,&K ,1*sizeof(dtype),0,cudaMemcpyHostToDevice )  ;
    cudaMemcpyToSymbol( drift ,&h_drift ,1*sizeof(dtype),0,cudaMemcpyHostToDevice )  ;
    cudaMemcpyToSymbol( diffusion,&h_diffusion ,1*sizeof(dtype),0,cudaMemcpyHostToDevice )  ;
