    ...
    public:
                    ...
                    #pragma acc parallel
                    {
                        ...
                        for(unsigned long long iter=start;iter<end;iter++)
                        {
                            ...
                            for(int t=1;t<time_step;t++)
                            {
                                ...
                                asset_price=asset_price*(1+R*dt+V*randn*sqrt(dt));
        ...
        */
                ...
                )
        {
            ...
            #pragma omp parallel reduction(+:payoff_sum) default(none) shared(sim_num,time_step,S0,K,T,V,R,dt,seed,is_input,payoff_array)
            {
                trng::yarn2 rng;
                rng.seed(seed);
...
class AmericanOption:public Option
{
    ...
    public:
                ...
                )
        {
            ...
            dt = T/static_cast<dtype>(time_step);
            dtype payoff_mat=new dtype[sim_num*time_step];
            dtype *payoff_array = new dtype[sim_num];
            for(int iter=0;iter<sim_num;iter++)
            {
                dtype asset_price = S0;
