...
class Option
{
    ...
    public:
                ...
                )
        {
            ...
            dtype *payoff_sum_array = new dtype[gpu_n]; //求和数组 // <--add
    
            #pragma omp parallel num_threads(gpu_n)
            {
                dtype payoff_sum=0.0; // <--add
                ...
                #pragma acc enter data create(payoff_array[start:end-start+1])
                {
                    #pragma acc parallel
                    {
                        ...
                        for(unsigned long long iter=start;iter<end;iter++)
                        {
                            ...
                            asset_price=asset_price*exp((R-0.5*V*V)*T+V*randn*sqrt(T));
                            dtype payoff;
                            if (!is_input)
                ...
                payoff_sum_array[rank]=payoff_sum; // <--add
            }
            
            dtype payoff_sum=0.0; // <--add
            for(int i=0;i<gpu_n;i++) // <--add
            { // <--add
                payoff_sum+=payoff_sum_array[i]; // <--add
            } // <--add
...
class AsianOption:public Option
{
    ...
    public:
                ...
                )
        {
            ...
            dtype *payoff_sum_array = new dtype[gpu_n];
            ...
            #pragma omp parallel num_threads(gpu_n)
            {
                ...
                */
                acc_wait_all();
                #pragma acc exit data copyout(payoff_array[start:end-start+1])
                payoff_sum_array[rank]=payoff_sum;
            }

            dtype payoff_sum=0.0;
            for(int i=0;i<gpu_n;i++)
            {
                payoff_sum+=payoff_sum_array[i];
            }
            price = payoff_sum/static_cast<dtype>(sim_num);
            price *= discount;
