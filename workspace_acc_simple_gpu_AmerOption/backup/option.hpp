    ...
    public:
                ...
                )
        {
            ...
            dtype *asset_mat=new dtype[4*8]{
                ...
                1.0,0.92,0.84,1.01,
                1.0,0.88,1.22,1.34
            }; //sim_num=8,time_step=4
            
            dt = T/static_cast<dtype>(time_step);
            dtype *cashflow_mat=new dtype[sim_num*time_step]; //cashflow矩阵
            ...
            for(int i=0;i<sim_num;i++)
            {
                ...
                {
                    dtype payoff;
                    dtype asset_price=asset_mat[i*time_step+t];
                    if (!is_input)
                        payoff=payoff_call(asset_price);
                    else
                        payoff=payoff_put(asset_price);
                    payoff_mat[i*time_step+t]=payoff;
                    cashflow_mat[i*time_step+t]=payoff;
                    //cout<<payoff_mat[i*time_step+t]<<" ";
                    printf("%.2f ",payoff_mat[i*time_step+t]);
                }
                cout<<endl;
            }
            cout<<endl;
            ...
            for(int iter=0;iter<sim_num;iter++)
            {
                ...
                for(int t=1;t<time_step;t++)
                {
                    ...
                    cashflow_mat[iter*time_step+t]=payoff;
                }
                //每条路径的每个时间节点都进行计算
            }
            */
            ...
            for(int t=time_step-2;t>=0;t--)
            {
                ...
                for(int i=0;i<sim_num;i++)
                {
                    ...
                    {
                        //cout<<"enter: "<<asset_mat[i*time_step+t]<<endl;
                        payoff_not_zero_x[num_non_zero]=asset_mat[i*time_step+t]; //当前点的股票价格
                        payoff_not_zero_y[num_non_zero]=payoff_mat[i*time_step+t+1]*exp(-R*dt);

                        cout<<"("<<payoff_not_zero_x[num_non_zero]<<", "<<payoff_not_zero_y[num_non_zero]<<")"<<endl;
                        num_non_zero++;
                    }
                }

                cout<<"t = "<<t<<" 时刻的拟合X-Y"<<endl;
                for(int i=0;i<num_non_zero;i++)
                {
                    cout<<"("<<payoff_not_zero_x[i]<<", "<<payoff_not_zero_y[i]<<")"<<endl;
                }
                cout<<endl;

                LeastSquareEigen ls(payoff_not_zero_x,payoff_not_zero_y,num_non_zero); //最小二乘拟合

                cout<<"t = "<<t<<" 时刻的X和估计值"<<endl;
                for(int i=0;i<num_non_zero;i++)
                {
                    cout<<"("<<payoff_not_zero_x[i]<<", "<<ls.get_Y(asset_mat[i*time_step+t])<<")"<<endl;
                }
                cout<<endl;
                //使用该拟合函数生成该时间段的持有收益
                for(int i=0;i<sim_num;i++)
                {
                    if(payoff_mat[i*time_step+t]>0)
                    {
                        ...
                        {
                            exec_time[i]=t;
                            for(int j=time_step-1;j>t;j--)  //决定当前时刻行权，后续的payoff全部归零
                            {
                                cashflow_mat[i*time_step+j]=0.0;
                            }
                        }
                        else
                        {
                            payoff_mat[i*time_step+t]=payoff_mat[i*time_step+t+1];
                            cashflow_mat[i*time_step+t]=0.0;
                        }
                    }
                    else
                        cashflow_mat[i*time_step+t]=0.0;
                ...
                cout<<"payoff_mat:"<<endl;
                for(int i=0;i<sim_num;i++)
                {
                    for(int j=0;j<time_step;j++)
                    {
                        printf("%.2f ",payoff_mat[i*time_step+j]);
                    }
                    cout<<endl;
                }
                cout<<endl;
                ...
                /*
                cout<<"cashflow_mat:"<<endl;
                for(int i=0;i<sim_num;i++)
                {
                    for(int j=0;j<time_step;j++)
                    {
                        printf("%.2f ",cashflow_mat[i*time_step+j]);
                    }
                    cout<<endl;
                }
                cout<<endl;
            ...
            for(int i=0;i<sim_num;i++)
            {
                ...
                cout<<exec_time[i]<<" ";
                cout<<payoff_mat[i*time_step+exec_time[i]]*exp(-R*dt*exec_time[i])<<endl;
            }
            cout<<endl;
