...
SimpleLeastSquare::SimpleLeastSquare(const dtype *x,const dtype *y, const int length)
{
...
dtype SimpleLeastSquare::get_Y(const dtype x)
{
    return a*x+b;
}
...
LeastSquareEigen::LeastSquareEigen(
        dtype *x,  //自变量值
        dtype *y,  //因变量值，要使用Eigen的Map，不能加const关键字
        const int length, //数据点个数
        const int order  //多项式阶数，默认为二阶多项式
        )
{
    this->order=order;
    int row=length; //4
...
dtype LeastSquareEigen::get_Y(const dtype x)
{
    dtype sum=0.0;
